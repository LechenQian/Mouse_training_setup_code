...
if __name__ == '__main__':
    ...
    print(file_path)
    mouse = Mouse_data(mouse_id=file_path[-5:], filedir = file_path[:-5])
    mouse.read_filename()
    #parse data
    ...
    if is_save:
        #save data by pickle
        #****************
        save_path = file_path+'/parsed_dataframe_pickle'
        
        filename = '{}_stats'.format(mouse.mouse_id)
    ...
    for index in range(len(all_days)):
        ...
        save_dir = os.path.join(file_path[:-5],'figures')
        # concatenate data
